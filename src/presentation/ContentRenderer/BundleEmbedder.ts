...
export class BundleEmbedder {
  ...
  public static getReactBundle(): string {
    ...
    try {
      const bundlePath = path.join(__dirname, '../../../dist/react-bundle.js');
      ...
      console.warn('Failed to load React bundle:', error);
    }

    // Fallback - минимальная реализация для разработки
    return this.getFallbackBundle();
  }

  private static getFallbackBundle(): string {
    return `
      // Fallback React implementation
      console.log('Using fallback React bundle');
      
      // Simple implementation of renderSqlPreview

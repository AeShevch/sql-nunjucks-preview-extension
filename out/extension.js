...
function activate(context) {
    try {
        ...
        const showPreviewCommand = vscode.commands.registerCommand('sqlNunjucksPreview.showPreview', () => {
        ...
        const showFullRenderCommand = vscode.commands.registerCommand('sqlNunjucksPreview.showFullRender', async () => {
            const editor = vscode.window.activeTextEditor;
            if (!editor || !editor.document.fileName.endsWith('.sql')) {
                vscode.window.showErrorMessage('Откройте SQL файл для предварительного просмотра');
                return;
...
class SqlNunjucksProcessor {
    ...
    showPreview(document, isFullRender = false) {
        ...
        if (panel) {
            ...
            panel = vscode.window.createWebviewPanel('sqlNunjucksPreview', title, vscode.ViewColumn.Beside, {
                enableScripts: true,
    ...
    async showFullRender(document) {
        ...
        let panel = this.webviewPanels.get(panelKey);
        if (!panel) {
            panel = vscode.window.createWebviewPanel('sqlNunjucksFullRender', `SQL Full Render - ${path.basename(document.fileName)}`, vscode.ViewColumn.Beside, {
                enableScripts: true,
                retainContextWhenHidden: true,
            });
            panel.onDidDispose(() => {
        ...
        try {
            ...
            panel._storedVariables = variables;
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Неизвестная ошибка';
            panel.webview.html = this.generateErrorContent(errorMessage);
    ...
    processIncludesRecursive(content, baseDir, workspaceRoot, processedFiles) {
        ...
        return content.replace(includeRegex, (match, includePath) => {
            ...
            try {
                ...
                const processedInclude = this.processIncludesRecursive(includeContent, sqlsBaseDir, workspaceRoot, new Set(processedFiles));
    ...
    generateWebviewContent(sql, fileName, isFullRender = false, variables) {
